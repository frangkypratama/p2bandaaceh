...
@section('content')
    ...
    <div class="container-lg">
        <form action="{{ route('lp.store') }}" method="POST" id="lpForm">
            @csrf
            <input type="hidden" name="lphp_id" value="{{ $lphp->id }}">

            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h4 class="card-title mb-0 d-flex align-items-center">
                        <i class="cil-plus me-2"></i>
                        <span><strong>Buat Laporan Pelanggaran (LP)</strong></span>
                    </h4>
                    <small class="text-medium-emphasis-white">Tindak lanjut atas LPHP {{ $lphp->nomor_lphp }}.</small>
                </div>
                            ...
                            <div class="card h-100 border-light shadow-sm">
                                ...
                                <div class="card-body">
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-6">
                                            <label class="form-label">Tanggal LPHP</label>
                                            <input type="text" class="form-control" value="{{ optional($lphp->tanggal_lphp)->translatedFormat('d F Y') }}" readonly>
                                        </div>
                                        <div class="col-md-6">
                                        ...
                                        <div class="col-md-6">
                                            <label class="form-label">Dugaan Pelanggaran</label>
                                            <input type="text" class="form-control" value="{{ $lphp->pasal }} - {{ $lphp->uu_terkait }}" readonly>
                                        </div>
                                        <div class="col-md-6">
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="date" class="form-control @error('tanggal_lp') is-invalid @enderror"
                                                   id="tanggal_lp" name="tanggal_lp" value="{{ old('tanggal_lp', optional($lphp->tanggal_lphp)->format('Y-m-d')) }}" required>
                                            @error('tanggal_lp')
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select @error('pejabat_penerbit_id') is-invalid @enderror" id="pejabat_penerbit_id" name="pejabat_penerbit_id" required>
                                                ...
                                                @foreach($petugasData as $petugas)
                                                    <option value="{{ $petugas->id }}" {{ old('pejabat_penerbit_id') == $petugas->id ? 'selected' : '' }}>{{ $petugas->nama }}</option>
                                                @endforeach
                                            </select>
