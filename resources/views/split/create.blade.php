...
@section('content')
    @php $lpp = $lpf->lpp; $lp = optional($lpp)->lp; $lphp = optional($lp)->lphp; $sbp = optional($lphp)->sbp; @endphp
    <div class="container-lg">
        <form action="{{ route('split.store') }}" method="POST" id="splitForm">
            @csrf
            <input type="hidden" name="lpf_id" value="{{ $lpf->id }}">

            <div class="card shadow-sm">
                <div class="card-header bg-primary text-white">
                    <h4 class="card-title mb-0 d-flex align-items-center">
                        <i class="cil-plus me-2"></i>
                        <span><strong>Buat Surat Perintah Penelitian (SPLIT)</strong></span>
                    </h4>
                    <small class="text-medium-emphasis-white">Tindak lanjut atas LPF {{ $lpf->nomor_lpf }}.</small>
                </div>
                                ...
                                <div class="card-body">
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" value="{{ $lpf->nomor_lpf }}" readonly>
                                        </div>
                                        <div class="col-md-6">
                                        ...
                                        <div class="col-md-6">
                                            <label class="form-label">Pekerjaan / Tempat &amp; Tanggal Lahir</label>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-12">
                                            ...
                                            <textarea class="form-control @error('dasar') is-invalid @enderror"
                                                      ...
                                                      required>{{ old('dasar', $defaultDasar) }}</textarea>
                                            @error('dasar')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-12">
                                            ...
                                            <textarea class="form-control @error('pertimbangan') is-invalid @enderror"
                                                      ...
                                                      required>{{ old('pertimbangan', $defaultPertimbangan) }}</textarea>
                                            @error('pertimbangan')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                        <div class="col-md-12">
                                            ...
                                            <textarea class="form-control @error('uraian_tugas') is-invalid @enderror"
                                                      id="uraian_tugas" name="uraian_tugas" rows="3">{{ old('uraian_tugas', $defaultUraianTugas) }}</textarea>
                                            @error('uraian_tugas')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select @error('petugas1_id') is-invalid @enderror" id="petugas1_id" name="petugas1_id" required>
                                                ...
                                                @foreach($petugasData as $petugas)
                                                    <option value="{{ $petugas->id }}" {{ old('petugas1_id') == $petugas->id ? 'selected' : '' }}>{{ $petugas->nama }} - {{ $petugas->jabatan }}</option>
                                                @endforeach
                                            </select>
                                            @error('petugas1_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select @error('petugas2_id') is-invalid @enderror" id="petugas2_id" name="petugas2_id" required>
                                                <option value="" disabled selected>Pilih Petugas...</option>
                                                @foreach($petugasData as $petugas)
                                                    <option value="{{ $petugas->id }}" {{ old('petugas2_id') == $petugas->id ? 'selected' : '' }}>{{ $petugas->nama }} - {{ $petugas->jabatan }}</option>
                                                @endforeach
                                            </select>
                                            @error('petugas2_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                    ...
                                    <div class="row g-3">
                                        <div class="col-md-6">
                                            ...
                                            <select class="form-select @error('penerbit_id') is-invalid @enderror" id="penerbit_id" name="penerbit_id" required>
                                                <option value="" disabled selected>Pilih Petugas...</option>
                                                @foreach($petugasData as $petugas)
                                                    <option value="{{ $petugas->id }}" {{ old('penerbit_id') == $petugas->id ? 'selected' : '' }}>{{ $petugas->nama }}</option>
                                                @endforeach
                                            </select>
                                            @error('penerbit_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
